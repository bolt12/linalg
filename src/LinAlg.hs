...
data L :: (Type -> Type) -> (Type -> Type) -> (Type -> Type) where
...
onesV :: (ToScalar a, Representable a, Semiring s) => L a Par1 s
...
fstM :: (Semiring s, V2 m k) => L (m :.: k) (m :.: Par1) s
fstM = cross (pureRep onesV)

sndM :: (Semiring s, V2 m k) => L (m :.: k) k s
sndM = JoinL (pureRep idL)

-- Khatri Rao product (Matrix pairing)

khatri :: (Semiring s, V3 f m k) => L f (m :.: Par1) s -> L f k s -> L f (m :.: k) s
